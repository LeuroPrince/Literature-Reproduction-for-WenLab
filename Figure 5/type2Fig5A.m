...
n_1 = 0;
n_2 = 0;
for i = 1:trial   %这里带来了数据的宽度，也就是为什么我们要平均
    for j = 1:size(time{i},2)
        ...
        if isnan(time{i}(3,j))
            ...
            tt = time{i}(1,j):time{i}(3,j);   %开始计算type2
            n_2 = n_2 + 1;
        end
        temp = min(smo{i}(tt));
...
%%
%%这一个代码块与type1中的不同之处在于我们不需要考虑太多边界问题，我们的分析总是在
%%reversal start -> reversalend（turn start）-> turn end这个时间窗口内
turn = cell(1,backtimemax);
back = cell(1,backtimemax);
for i = 1:trial
    for j = 1:size(time{i},2)
        if isnan(time{i}(1,j)) == 0
            if isnan(time{i}(3,j)) == 0   %差异所在，在这里我们选择了第三行不为0的数据
